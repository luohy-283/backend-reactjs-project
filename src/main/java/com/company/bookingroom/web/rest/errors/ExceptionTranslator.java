package com.company.bookingroom.web.rest.errors;
...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
        if (
            ex instanceof com.company.bookingroom.service.UsernameAlreadyUsedException
        ) return (ProblemDetailWithCause) new LoginAlreadyUsedException().getBody();
        if (
            ex instanceof com.company.bookingroom.service.EmailAlreadyUsedException
        ) return (ProblemDetailWithCause) new EmailAlreadyUsedException().getBody();
        if (
            ex instanceof com.company.bookingroom.service.InvalidPasswordException
    ...
    protected ProblemDetailWithCause customizeProblem(ProblemDetailWithCause problem, Throwable err, NativeWebRequest request) {
        ...
        // Keep BadRequestAlertException title (business message); do not replace with "Bad Request"
        String title = extractTitle(err, problem.getStatus());
        String problemTitle = problem.getTitle();
        if (problemTitle == null) {
            problem.setTitle(title);
        } else if (!(err instanceof BadRequestAlertException) && !problemTitle.equals(title)) {
            problem.setTitle(title);
        }
